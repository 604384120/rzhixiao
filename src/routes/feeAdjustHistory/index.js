...
const FeeAdjustHistory = ({
  ...
  const userSortProps = {
    ...
    onUpdate (data) {
      dispatch({type: 'feeAdjustHistory/updateSort'})  // 更多筛选加蒙版
      dispatch({
        type: 'app/updateDisplay',
        payload: {
          displayListTemp:data,
  ...
  const structSelectProps = {
    ...
    onChangeSort (value, attr) {
      dispatch({type: 'feeAdjustHistory/updateSort'})  // 加蒙版
    },
  }
  ...
  const handleChangeYear  = (value) => {
    dispatch({type: 'feeAdjustHistory/updateSort',payload:{year: value}})    //学年加蒙版
  }
  ...
  const handleChangeMission = (value) => {
    dispatch({type: 'feeAdjustHistory/updateSort',payload:{missionId: value}})    //任务名称加蒙版
	}

	const handleChangeSubject = (value) => {
    dispatch({type: 'feeAdjustHistory/updateSort',payload:{subjectId: value}})    //项目名称加蒙版
	}
  ...
  const handleResetQuery = () => {
    ...
    dispatch({type: 'feeAdjustHistory/updateSort',    //重置加蒙版
      payload: {
        userSortList,
        accountId: [],
        subjectId:undefined,
          ...
          <Card bordered={false} bodyStyle={{ padding: ' 0' }}>
            ...
            <div style={{ padding: '5px', textAlign: 'center' }}>
              <Row style={{fontSize: '15px'}}>
                <Col span={12} >{'应收调整总额：'+getFormat(feeAdjustSum?feeAdjustSum:0)+'元'}</Col>
                <Col span={12} >{'应收调整记录：'+(feeAdjustCount?feeAdjustCount:0)+'笔'}</Col>
              </Row>
            </div>
            <Divider style={{ margin: '5px' }} dashed />
