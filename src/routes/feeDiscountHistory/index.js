...
const FeeDiscountHistory = ({
  location, dispatch, feeDiscountHistory, loading, app
}) => {
  const { isNavbar } = app;
  const {
    ...
    dataList, feeDiscountSum, feeDiscountCount, orderBy,
  } = feeDiscountHistory
  ...
  const userSortProps = {
    ...
    onUpdate (data) {
      dispatch({type: 'feeDiscountHistory/updateSort'})  // 更多筛选加蒙版
      dispatch({
        type: 'app/updateDisplay',
        payload: {
          displayListTemp:data,
  ...
  const structSelectProps = {
    ...
    onChangeSort (value, attr) {
      dispatch({type: 'feeDiscountHistory/updateSort'})  // 加蒙版
    },
  }
  ...
  const userTableProps = {
    ...
    onChangePage (n, s) {
      dispatch({
        type: 'feeDiscountHistory/getFeeDiscountHistory',
        payload: {
          ...queryParam,
          pageNum: n,
          pageSize: s,
  ...
  const handleChangeYear  = (value) => {
    dispatch({type: 'feeDiscountHistory/updateSort',payload:{year: value}})    //学年加蒙版
  }

  const handleChangeDate = (value) => {
    dispatch({
      type: 'feeDiscountHistory/updateSort',
      payload:{
  ...
  const handleChangeMission = (value) => {
    dispatch({type: 'feeDiscountHistory/updateSort',payload:{missionId: value}})    //任务名称加蒙版
	}

	const handleChangeSubject = (value) => {
    dispatch({type: 'feeDiscountHistory/updateSort',payload:{subjectId: value}})    //项目名称加蒙版
  }
  
  // const handleChangeStatus = (value) => {
  //   dispatch({type: 'feeDiscountHistory/updateSort',payload:{status: value}})    //状态加蒙版
  // }
  ...
  const handleResetQuery = () => {
    ...
    dispatch({type: 'feeDiscountHistory/updateSort',    //重置加蒙版
      payload: {
        userSortList,
        accountId: [],
        subjectId:undefined,
  ...
  const handleChangeSearchName = (value) =>{
    dispatch({
      type: 'feeDiscountHistory/updateState',
      payload: {
          ...
          <Card bordered={false} bodyStyle={{ padding: ' 0' }}>
            ...
            <div style={{ padding: '5px', textAlign: 'center' }}>
              <Row style={{fontSize: '15px'}}>
                <Col span={12} >{'减免调整总额：'+getFormat(feeDiscountSum?feeDiscountSum:0)+'元'}</Col>
                <Col span={12} >{'减免调整记录：'+(feeDiscountCount?feeDiscountCount:0)+'笔'}</Col>
              </Row>
            </div>
            <Divider style={{ margin: '5px' }} dashed />
...
FeeDiscountHistory.propTypes = {
  feeDiscountHistory: PropTypes.object,
  location: PropTypes.object,
  dispatch: PropTypes.func,
  loading: PropTypes.object,
}

export default connect(({ feeDiscountHistory, app, loading }) => ({ feeDiscountHistory, app, loading }))(FeeDiscountHistory)
