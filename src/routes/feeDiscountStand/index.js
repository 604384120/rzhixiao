...
const FeeDiscountStand = ({
  ...
  const tableProps = {
    ...
    onUpdateState: (data) => {
      dispatch({
        type: 'feeDiscountStand/updateState',
        payload: {
          ...data,
        },
      })
    ...
    onSave: (record) => {
      ...
      if (record.id) {
        data.id = record.id;
        dispatch({
          type: 'feeDiscountStand/updateDiscountStand',
          payload: {
            ...data,
          },
        })
        ...
        dispatch({
          type: 'feeDiscountStand/addDiscountStand',
          payload: {
            ...data,
          },
        })
  ...
  const handleClickSort = () => {
    if(!departTreeList || departTreeList.length<=0){
      dispatch({
        type: 'feeDiscountStand/getDepartTreeList',
      })
    }
  }
  ...
  const handleChangeDepart = (value) => {
    dispatch({
      type: 'feeDiscountStand/getSubjectList',
      payload:{
        departId: value,
      },
    })
  ...
  const handleSearch = (value) => {
    dispatch({type: 'feeDiscountStand/updateState',payload:{searchName: value}})
  }
  ...
  const handleChangeSubject = (index) => {
    ...
    dispatch({
      type: 'feeDiscountStand/getDataList',
      payload:{
        subjectId: index
      },
    })
...
export default connect(({ feeDiscountStand, app, loading }) => ({ feeDiscountStand, app, loading }))(FeeDiscountStand)
