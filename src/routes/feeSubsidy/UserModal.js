			...
			if(modalImportData.step == 0){
				...
				return (
					<div>
						...
						<Row  style={{margin: 'auto', textAlign:'center'}}>
							<div style={{fontSize:'12px', marginBottom:'30px'}}>请先<a target="_blank" href={modalImportData.importType==1?dowloadUrl(api.downloadSubsidyTemp):dowloadUrl(api.downloadFeeSubsidyTemp)}>下载标准模板</a>并按照表格说明整理表格，否则可能导致上传失败</div>
				...
				return (
					<div style={{ margin: 'auto', textAlign:'center', paddingTop:'20px' }}>
						...
						<div style={{ marginTop: '30px' }}>
						<span>{modalImportData.cfNum}条重复数据导入失败 {modalImportData.importing || modalImportData.cfNum == '0' ? '' : <a target="_blank" href={dowloadUrl(api.exportRepetitionOrder)}>下载</a>} {renderCoverButton()}</span>
						</div>
					</div>
				)
			}
