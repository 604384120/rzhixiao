import { Modal, Button, message, Row, InputNumber, Input, Spin, Icon, Tag, Upload,Popconfirm, Cascader, Select, Col } from "antd";
...
const UserModal = ({
  ...
  onSubmit
    ...
    const { attrMap, payType } = modalData
    const handleSubmit = () => {
      ...
      if(modalData.fee && modalData.fee != '0' && !payType){
      ...
      if(modalData.fee){
        param.fee = modalData.fee
      }
      if(modalData.feeName){
        param.feeName = modalData.feeName
      }
      param.type = '2'  //  意向报名
      onSubmit(param)
    }
    ...
    const renderTags = () => {
      ...
      for (let payType of payTypeList) {
        ...
        tags.push(
          <Tag id={payType.payType} color={(payType.code)?'#d2d2d2':modalData.payType==payType.payType?'#108ee9':''} onClick={(payType.code)?undefined:()=>{handleChagePayType(payType.payType)}}>{payType.name}</Tag>
        )
      }
    ...
    const renderAttr = () => {
      ...
      return list
        ...
        <div style={{maxWidth:'500px',margin:'auto'}}>
          ...
          {modalData.fee && modalData.fee != '0' && <Row style={{padding: '20px 0',}}>
            <Col span={12}>
              <span>需缴纳{modalData.feeName}：{getFormat(modalData.fee)}元</span>
            </Col>
            <Col span={12} style={{textAlign: 'right'}}>支付方式：
              {renderTags()}
            </Col>
          </Row>}
