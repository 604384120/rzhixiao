...
const JoinUserInfo = ({
  ...
  const { isNavbar } = app
  const {
    typeMap, tempTypeMap, attrMap, joinUserInfoMap, userId, opHistoryVisible, departTree, departMap,disabledAttr,
  ...
  const handleEditGroup = (type) => {
    ...
    for (let node of tempTypeMap[type].list) {
      if (joinUserInfoMap[node.id]) {
        if (node.valueType == '1') {
          tempTypeMap[type]._changeTemp[node.id] = joinUserInfoMap[node.id].relateName
        } else {
          tempTypeMap[type]._changeTemp[node.id] = joinUserInfoMap[node.id].relateId
        }
      }
    }
    dispatch({
  ...
  const handleSaveGroup = (type) => {
    dispatch({
      type: 'joinUserInfo/updateJoinUserInfo',
      payload: {
        id: userId,
        type,
      },
  ...
  const handleChangeOpHistoryVisible = (visible) => {
    dispatch({
      type: 'joinUserInfo/changeOpHistoryVisible',
      payload: {
        visible,
      },
    })
  ...
  const handleClickAttrValueOption = (attr) => {
    dispatch({
      type: 'joinUserInfo/getUserAttrValue',
      payload: {
        id: attr.id,
      },
    })
  ...
  const handleLoadData = (selectedOptions) => {
    ...
    dispatch({
      type: 'joinUserInfo/getStructItemList',
      payload: {
        pid: targetOption.id,
      },
    })
  ...
  const showAttrValueName = (type, attr) => {
    ...
    return joinUserInfoMap[attr.id] ? joinUserInfoMap[attr.id].relateName : undefined
  }
  ...
  const createAttrValue = (type, attr) => {
    ...
    return joinUserInfoMap[attr.id] ? joinUserInfoMap[attr.id].relateName : ''
  }
  ...
  const handleInfiniteOnLoad = () => {
    ...
    dispatch({
      type: 'joinUserInfo/getUserOperate',
      payload: {
        ...opQueryParam,
      },
    })
  ...
  const renderContent = (item) => {
    ...
    return (
    <Row>{str} {item.info&&item.info.length>0?<a onClick={(e) => { showSnapshotModal(item) }}>点此查看</a>:""}</Row>
    )
  }
...
JoinUserInfo.propTypes = {
  joinUserInfo: PropTypes.object,
...
export default connect(({ joinUserInfo,app, loading }) => ({ joinUserInfo,app, loading }))(JoinUserInfo)
