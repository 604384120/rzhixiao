...
const OrderTable = ({
  orderData,
    ...
    orderData.selectedOrders = selectedRowKeys
	...
	const rowSelection = {
		selectedRowKeys: orderData.selectedOrders,
	...
	return (
		<div>
		  <TableCom
			...
			columns={columns}
			pagination={false}
			loading={orderData.dataLoading}
			rowKey={record => record.orderNo}
			className={styles.fixedTable}
			scroll={{x:num*80}}
			styles={styles}
	      />
		  {orderData.count>0&&<Pagination style={{float:"right",marginTop: '20px'}} current={orderData.pageNum} defaultPageSize={orderData.pageSize} onChange={handleChangePage} onShowSizeChange={handleChangeSize}
		  	 total={orderData.count} showTotal={(count) => `共 ${count} 条`} showSizeChanger showQuickJumper/>}
		</div>
		)
}
